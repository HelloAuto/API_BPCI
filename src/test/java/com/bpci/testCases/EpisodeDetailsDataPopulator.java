...
public class EpisodeDetailsDataPopulator extends TestBase {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println(rsSet);
				 ...
				 String admitDate = (String)rs.getString(1);
				 System.out.println("admitDate--  "+admitDate);
				
				 
				 String dischargeDate = (String) rs.getString(2);
	...
	public void vailidateAPIResult(List<Map<String,String>> responseBody, ArrayList<HashMap<String, String>> queryResultList, String excelPath
		...
		for(HashMap<String, String> rsSet : queryResultList) {
			...
			Object curResponse = responseBody.get(listIndex).get(colName);//for checking integer case error
				 responseBody.get(listIndex).get(colName);
